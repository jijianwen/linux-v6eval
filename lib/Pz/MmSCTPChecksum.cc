...
#define SCTP_HDR_LEN		(3 * sizeof(uint32_t))

static uint32_t sctp_start_cksum(uint8_t *buffer, uint16_t length)
{
	...
	for (i = 0; i < (SCTP_HDR_LEN - sizeof(uint32_t)); i++)
	...
	for (i = SCTP_HDR_LEN; i < length ; i++)
		CRC32C(crc32, buffer[i]);

	return crc32;
}

static uint32_t sctp_end_cksum(uint32_t crc32)
{
	uint32_t result;
...
MmSCTPChecksum::MmSCTPChecksum(CSTR s,uint16_t w,const ICVoverwriter* ow):SUPER(s,w,0,0,ow){}
MmSCTPChecksum::~MmSCTPChecksum(){}

RObject* MmSCTPChecksum::reverse(RControl& c,
...
bool MmSCTPChecksum::geneAuto(WControl& c,WObject* w_self,OCTBUF&)const{
	if(!c.error()){
...
void MmSCTPChecksum::add_post(Con_IPinfo* info,TObject* self)const{
	if(info)info->postUppChecksum(self);
}

WObject* MmSCTPChecksum::composeWm(WControl&,
		WObject* w_parent,const PObject* po)const {
	WmSCTPChecksum* w_self = new WmSCTPChecksum(w_parent,this,po);
	return w_self;
}

RObject* MmSCTPChecksum::reverseRm(RControl&,RObject* r_parent,
		const ItPosition& at,const ItPosition& size,PvObject* pv)const {
	RmSCTPChecksum* r_self = new RmSCTPChecksum(r_parent,this,at,size,pv);
	return r_self;
}
...
RmSCTPChecksum::RmSCTPChecksum(RObject* r_parent,const MObject* m,
		const ItPosition& offset,const ItPosition& size,PvObject* pv):
		SUPER(r_parent,m,offset,size,pv),
		calc_pvalue_(new OCTBUF()) {}

RmSCTPChecksum::~RmSCTPChecksum(){
	if(calc_pvalue_){delete calc_pvalue_; calc_pvalue_=0;}
}

void RmSCTPChecksum::set_calc_pvalue(PvObject* calc) {
	if(calc_pvalue_){delete calc_pvalue_;}
	calc_pvalue_=calc;
}

void RmSCTPChecksum::post_reverse(Con_IPinfo& info,RControl& c,RObject* base){
	OCTBUF* basebuf = (OCTBUF*)base->pvalue();
	OCTBUF checkbuf(basebuf->length(),(OCTSTR)basebuf->string(),true);

	uint32_t calc = sctp_start_cksum((uint8_t *)checkbuf.string(), checkbuf.length());
	calc = sctp_end_cksum(calc);
...
void RmSCTPChecksum::printName(uint32_t t,CSTR cls) const {
	SUPER::printName(t,cls);
	printf(" calc("); if(calc_pvalue_)calc_pvalue_->print(); printf(")");
}

void RmSCTPChecksum::logSelf(uint32_t t,CSTR cls) const {
	SUPER::logSelf(t,cls);
	printf(" calc("); if(calc_pvalue_)calc_pvalue_->print(); printf(")");
}
...
WmSCTPChecksum::WmSCTPChecksum(WObject* p,const MObject* m,const PObject* po):SUPER(p,m,po){}
WmSCTPChecksum::~WmSCTPChecksum(){}

void WmSCTPChecksum::post_generate(Con_IPinfo& info,WControl& c,OCTBUF& buf,
		WObject* base){
...
bool WmSCTPChecksum::doEvaluate(WControl& c,RObject& r) {
	//typecheck be finished by metaEvaluate()
	RmSCTPChecksum& rm = (RmSCTPChecksum &)r;
